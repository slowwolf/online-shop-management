<?php
	...
	if(isset($_POST['login'])) {
		...
		$row = $conn->query("SELECT * FROM users WHERE username='".$username."' AND password='".$password."'")->fetch_array();
		if(!empty($row['id'])) {
			setcookie("clt", $row['token'], time() + (86400 * 2), "/");
			header("Location: index.php");
		} else $emsg = "Invalid username or password !";
	}
?>
<html>
   <head>
							...
							<form  method="POST" action="">
								<input type="hidden" name="login">
									<div class="form-group">
										...
										<input name="username" type="text" class="form-control" placeholder="Enter Email Address" required="">
									</div>
									<div class="form-group">
										<label>Password  </label>
										<input name="password" type="password" class="form-control" placeholder="Enter Password" required="">
									</div>
									<div class="form-group"><input type="checkbox"> Remember me</div>
									
									<button type="submit" class="form-control btn-block">Login</button><br>
									<div>
										<?php if(isset($emsg)) { ?><p class="error-msg"><?= $emsg ?></p><?php }?>
										<h6>Not a register?</h6>
